<template>
  ...
  <div v-show="edit" class="container">
    <form name="update" ref="update" @submit.prevent="updateCust">
      <h3 class="text-yellow-800">Update this Customer</h3>
      <div class="mt-8 mb-6 grid grid-cols-3 gap-8">
        ...
        <button class="rounded ml-4 px-2 py-1 text-gray-100 bg-yellow-600 hover:bg-yellow-700" type="submit">Save</button>
    </form>
  </div>
...
export default {
  ...
  methods: {
    ...
    async updateCust () { // Update customer operations
      /* Reset all */
      var body = {}
      ...
      if (!this.updateErrorFlag) {
        await CustomerService.updateCustomer(this.customer._id, body)
